...
fish <- fish %>%
  mutate(length = measurement_check(length, 5, 80),
         weight = measurement_check(weight, 10, 4000))
```
...
{
  df <- df %>%
    mutate(condition_index = {{weight}} / ({{length}} ^ 3))
...
fish %>%
  ...
  theme_bw() +
  labs(
    title = "Graph of Mean Condition Index by Year and Species",
    x = "Year",
    y = "",
    subtitle = "Mean of Condition Index",
    color = "Species"
  )
```
